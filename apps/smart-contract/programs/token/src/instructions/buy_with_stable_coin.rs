    ...
    pub fn process_instruction(ctx: Context<Self>, mut stable_coin_amount: u64) -> Result<()> {
        ...
        if global_state.is_live == false {
            match stage_iterator {
                0 => return Err(error!(TokenError::TokenNotStarted)),
                stage if stage > NUM_STAGES => return Err(error!(TokenError::TokenEnded)),
                _ => return Err(error!(TokenError::TokenPaused)),
            }
        }


        //  check stable coin address
        require!(ctx.accounts.stable_coin.key() == USDC_ADDRESS || ctx.accounts.stable_coin.key() == USDT_ADDRESS, TokenError::InvalidStableToken);
