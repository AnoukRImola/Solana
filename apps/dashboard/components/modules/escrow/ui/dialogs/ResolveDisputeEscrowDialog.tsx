...
const ResolveDisputeEscrowDialog = ({
	...
	useEffect(() => {
		const platformFee = Number.parseFloat(escrow?.platformFee || '0') / 100
		const parsedApproverFunds = Number.parseFloat(approverFunds) || 0
		const parsedReceiverFunds = Number.parseFloat(receiverFunds) || 0

		setTotalPlatformAmount(
			selectedEscrow?.amount && !Number.isNaN(Number(selectedEscrow.amount))
		...
		if (
			Number.isNaN(parsedApproverFunds) ||
			Number.isNaN(parsedReceiverFunds) ||
		...
		setIsEqualToAmount(
			parsedApproverFunds + parsedReceiverFunds ===
				Number.parseFloat(escrow?.balance || '0'),
		)
