...
import type { DashboardData } from '../@types/dashboard.entity'
...
const getReleaseTrend = (escrows: Escrow[]) => {
	...
	escrows.forEach((escrow) => {
		if (escrow.releaseFlag && escrow.updatedAt?.seconds) {
			const month = format(new Date(escrow.updatedAt.seconds * 1000), 'yyyy-MM')
...
const getVolumeTrend = (escrows: Escrow[]) => {
	...
	escrows.forEach((escrow) => {
		const date = format(new Date(escrow.createdAt.seconds * 1000), 'yyyy-MM-dd')
		const value = Number.parseFloat(escrow.amount)
