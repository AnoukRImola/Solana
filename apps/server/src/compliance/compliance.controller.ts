...
export class ComplianceController {
  ...
  @UseGuards(AuthGuard())
  @ApiBearerAuth('jwt-auth')
  async closeRegistry(@Body('signer') signer: string): Promise<ApiResponse> {
    try {
      return await this.complianceService.closeComplianceRegistry(signer);
    } catch (error) {
      if (error instanceof HttpException) throw error;
      throw new HttpException(
        {
        ...
        throw new HttpException(
          {
            ...
            message: 'Compliance registry not initialized',
          },
  ...
  async getVerification(@Query() dto: GetVerificationDto) {
    try {
      const verification = await this.complianceService.getAddressVerification(
        dto.address,
      );
      if (!verification) {
        throw new HttpException(
