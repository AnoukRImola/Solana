...
export class HelperController {
  ...
  async sendTransaction(
    ...
    const { signedXdr, returnEscrowDataIsRequired, saveInfo } =
      sendTransactionDto;
    try {
      const transactionSigned = await this.helperService.sendTransaction(
        signedXdr,
  ...
  async getMultipleEscrowBalance(
    ...
    try {
      ...
      const balance = await this.helperService.getMultipleEscrowBalance(
        signer,
        addresses,
      );
      return balance;
